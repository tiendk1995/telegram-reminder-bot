...
const chatIdFLReport = process.env.TELEGRAM_CHAT_ID_FL_REPORT || chatIdAssignedOrders;
...
if (testType === 'evening') {
  ...
  targetChatId = chatIdAfternoon;
} else if (testType === 'checkin') {
  targetChatId = chatIdCheckin;
}
...
if (testType === 'evening') {
  ...
  console.log('Đang chạy script quét GHN checkin...');
  exec(`python "${scriptPath}"`, (error, stdout, stderr) => {
    if (error) {
    ...
    if (fs.existsSync(photoPath)) {
      if (finalReport.length > 1024) {
        bot.sendPhoto(targetChatId, photoPath, { caption: `⏱️ <b>Báo cáo Check-in trễ hôm nay (Xem chi tiết ở tin nhắn dưới)</b>`, parse_mode: 'HTML' })
          .then(() => {
            bot.sendMessage(targetChatId, finalReport, { parse_mode: 'HTML' })
              .then(() => {
                console.log(`✅ GỬI TIN NHẮN THỬ NGHIỆM CHECKIN THÀNH CÔNG!`);
                process.exit(0);
              })
              .catch((err) => {
                console.error('Lỗi gửi text checkin sau ảnh:', err.message);
                process.exit(1);
              });
          })
          .catch((err) => {
            console.error('Lỗi gửi ảnh checkin:', err.message);
            process.exit(1);
          });
      } else {
        bot.sendPhoto(targetChatId, photoPath, { caption: finalReport, parse_mode: 'HTML' })
          .then((resMsg) => {
            console.log(`✅ GỬI TIN NHẮN THỬ NGHIỆM CHECKIN THÀNH CÔNG!`);
            process.exit(0);
          })
      ...
      bot.sendMessage(targetChatId, finalReport, { parse_mode: 'HTML' })
        ...
        .catch((err) => {
          console.error('Lỗi gửi text checkin:', err.message);
          process.exit(1);
        });
    }
  });
